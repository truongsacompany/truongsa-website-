...
$(function(){
	...
	$(window).trigger('resize');
		...
		function render_pager_bk(p_id_parrent, p_current_page, id_pager){

			var p_page_size =12;

			var list_item = $('#'+p_id_parrent).find('li');
			var total_item = list_item.length;
			var total_page = Math.round(total_item/p_page_size);
			if(total_page * p_page_size < total_item)
			...
			{
				if(i==p_current_page)
					html_pager+='<li class="active"><span>' + i + '</span></li>'
					else
					{
						html_pager+='<li><a href="" onclick="render_pager(\'' + p_id_parrent + '\',' + i + ', '+ '\'' + id_pager + '\'); return false;">'+ i +'</a></li>'
					}
			}

			$('#'+id_pager).html(html_pager);
			//alert(html_pager);

		

			for(var j=1; j<=total_item; j++)
			{
				if(j > p_page_size * (p_current_page -1) && j <= p_current_page * p_page_size )
					$(list_item[j-1]).show();
				else
					$(list_item[j-1]).hide();
			}
		...
		function render_pager(p_id_parrent, p_current_page, id_pager){
			...
			var html_pager = '<li><a href="" onclick="render_pager(\'' + p_id_parrent + '\',1, '+ '\'' + id_pager + '\'); return false;">'+ '<<' +'</a></li>';
			
			var prev_page = p_current_page - 1;
			if(prev_page==0)
				prev_page = 1;

			html_pager += '<li><a href="" onclick="render_pager(\'' + p_id_parrent + '\',' + prev_page +  ', '+ '\'' + id_pager + '\'); return false;">'+ '<' +'</a></li>';
			...
			if(total_page>6)
			{
						...
						if(p_current_page>3 & p_current_page<total_page-2)
								html_pager+='<li class="active"><span>...</span></li>'
						else
							  html_pager+='<li><span>...</span></li>'
			...
			var next_page = p_current_page + 1;
			if(next_page>total_page)
				next_page = total_page;
			html_pager += '<li><a href="" onclick="render_pager(\'' + p_id_parrent + '\',' + next_page +  ', '+ '\'' + id_pager + '\'); return false;">'+ '>' +'</a></li>';
		
			html_pager += '<li><a href="" onclick="render_pager(\'' + p_id_parrent + '\',' + total_page +  ', '+ '\'' + id_pager + '\'); return false;">'+ '>>' +'</a></li>';
		...
		$(document).ready(function(){
			var total_tab = $('.tab-content .tab').length;
			for(var j=1; j<=total_tab; j++)
			{
				render_pager('tab'+j+'_list', 1, 'tab'+j+'_pager');
			}
